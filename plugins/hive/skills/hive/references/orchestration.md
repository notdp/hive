...
代理本身只做机械转发,`model: 'haiku'` 压住包装成本。注意 Workflow 面板的 Model 列显示的是**代理**的模型,成员真身的 CLI/模型没有任何接口能注入该列——唯一的显示杠杆是 label 自由文本。约定:`⬡ <name> · <cli>`,显式指定了成员模型时再补一段,如 `⬡ impl-auth · codex · gpt-5.4`(⬡ 标记 hive 节点,替代前缀)。
