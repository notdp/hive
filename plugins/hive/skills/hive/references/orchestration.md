...
开工:编排的 Claude session 里 `hive create <run>`,你以 `<run>.orch` 入册,session=team=run 名。hclaude 自动进入团窗口;桌面 Claude 在团窗口显示只读镜像,按 SKILL.md 入口一节在 Terminal 面板跑一次 `hive attach <run>` 给 human 看全场。
...
- 给 human 的可执行命令各放一个 ```bash 块,块内只放命令;`hive attach <team>` 桌面 Claude 在建团时自己跑(SKILL.md 入口一节),不作为命令块给 human。
