...
4. 验收对象是 commit:只提交本任务范围,让任务发送者按 commit 判;WIP commit 可以。任务说了不用 commit 就不 commit,交修改路径和验证结果。
5. 按任务要求开 PR、push、`gh pr ready`;merge 只由任务发送者执行。任务没提 PR 时,只推空 commit 建 draft PR 占位。
6. 任务发送者验过、或让你收摊后再退场。运行 `hive worktree done <task>`,只删 worktree,branch 留给 PR 生命周期。只对 claude:done 前先 `ExitWorktree action=keep`。人还在 worktree 里或有未提交改动时,`done` 会拒绝。worktree 是交付证据,不要提前删。被打回要再改就重跑 `start`,同一 branch 会挂回同一路径。`done --force` 只在 human 明确 abandon 时用,它连未合的工作一起丢。
