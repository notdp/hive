...
Use `hclaude`, `hcodex`, or `hgrok` (equivalently `hive claude`, `hive codex`, or `hive grok`) to start a managed conversation. Outside tmux, these launchers show it in the current terminal. Creating or joining a team moves the same session's viewer into the team window and opens that window here; chatting alone creates no tmux session. Detaching returns to the shell, and resuming an enrolled session opens its team display. A bare terminal CLI must first be resumed through its managed launcher. The desktop app's Claude session joins directly, with a read-only mirror in the team window.
...
- [`docs/runtime-model.md`](docs/runtime-model.md) — registry-vs-display identity, the per-CLI native runtime sources, and `busy` / `inputState`
