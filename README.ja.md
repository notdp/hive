...
ミラーはウィンドウ左端の細いレール (rail) として描かれます: 幅 14 列で、`hive view` はこの幅ではトランスクリプトの代わりにステータス列 (メンバー名、busy / idle、開いてから受け取った HIVE メッセージ数、最後のメッセージの経過時間と冒頭の数語) を描きます。レールをクリックすると 45% まで広がり、もう一度クリックすると折りたたまれます。`prefix+z` によるズームは他の pane と同じです。`hive mirror on|off` で表示・非表示を切り替えます (引数なしならトグル)。レールを描かないのは 1 つのヒューリスティックだけです: Claude デスクトップのターミナルパネル (`TERM_PROGRAM=claude-desktop`、stdin が tty) から `hive create` / `join` / `attach` (または失われたウィンドウを再構築する `hive spawn`) を実行し、その cwd がセッション自身の cwd と一致するとき、人はすでにその会話を見ているので、ウィンドウに `@hive-mirror off` を記録します。`attach` が見つけたウィンドウにそのセッションのレールがまだ出ていれば、同じ理由で一度だけ取り下げます。ウィンドウに記録がないときだけ働き、`hive mirror on` で上書きできます。
