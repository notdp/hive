...
`hclaude` / `hcodex` / `hgrok`（`hive claude` / `hive codex` / `hive grok` と同じ）で管理された会話を始めます。tmux の外では、これらのランチャーは現在のターミナルにそのまま表示します。チームの作成や参加は同じセッションのビューアをチームウィンドウへ移し、そのウィンドウをここで開きます。単独のチャットでは tmux セッションは作られません。デタッチするとシェルに戻り、登録済みセッションを再開するとそのチーム表示が開きます。素のターミナル CLI は、まず管理ランチャー経由で再開する必要があります。デスクトップアプリの Claude セッションは直接参加し、チームウィンドウには読み取り専用のミラーが表示されます。
