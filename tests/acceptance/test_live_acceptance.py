...
oracle: the tests hold it against the ledger, the node record under
`run/workflow/`, and the one transcript fact that still matters — the
dispatch landed in the member's conversation exactly once.
"""
...
def test_every_node_completed(rig):
    ...
    for cli in rig.clis:
        ...
        for stale in ("session", "turn", "artifact"):
...
def test_ledger_holds_one_dispatch_and_no_reply(rig):
    ...
    # await — a node member sends nothing back at all; its result is read
    # off its engine, not a bus row
    for cli in rig.clis:
        member = rig.member(cli)
        ...
        assert not sent, (
            ...
            f"(to {[r[2] for r in sent]}) — a node task's result is the member's last message, not a send"
        )


def test_record_terminal_and_body_matches(rig):
    # bought by: a record still pending after a completed verdict keeps the
    # name owned forever; a record body drifting from the node's line would
    # mean two truths about one turn
    for cli in rig.clis:
        member = rig.member(cli)
        ...
        assert record.get("body") == rig.node_results[member].get("body"), (
            f"{member}: record body differs from the node's line"
        )
...
def test_semantic_coroner(rig):
    ...
    rubric = (
        "你是 hive 多 agent runtime 的验收验尸官。下面是一次验收跑的材料(bus 消息行/各成员这一轮最后说的话即返回/node 日志/成员 pane 屏幕)。"
        "每个成员收到的是一封没有 from 的 workflow 任务信封(bus 里 from 为空的那行);它的结果就是它这一轮最后说的话,由引擎直接读走,成员不该寄任何信、不该跑任何返回命令。"
