"""Tests for `hive crew cleanup`.

Cleanup scans current crew's `<main>-cell-<N>` teams, kills their tmux
windows, and clears leftover `@hive-*` window options. Invariants:
  - no flags / positional args (timing is orch-skill-enforced, not CLI)
  - must run from a crew pane; running from a peer pane is rejected
  - main crew window (orch / challenger) is never touched
...
from hive.cli import _is_cell_team_name, cli


# --- _is_cell_team_name helper ---
...
@pytest.mark.parametrize(
    ...
    [
        ("613-6", False),
        ("613-6-cell-1", True),
        ("613-6-cell-1000", True),
        ("613-6-cell-1002", True),
        ("613-6-cell-", False),       # empty suffix
        ("613-6-cell-abc", False),    # non-numeric suffix
        ("peer-1000", False),         # no `-cell-` infix, only prefix
        ("team-cell-xyz-1000", False),  # `-cell-` exists but trailing suffix isn't digits
        ("613-6-cell-1-cell-2", True),  # nested `-cell-<N>`; rightmost wins
        ("", False),
    ],
)
def test_is_cell_team_name(name, expected):
    assert _is_cell_team_name(name) is expected
...
def test_cleanup_help_has_no_flags(runner):
    ...
    result = runner.invoke(cli, ["crew", "cleanup", "--help"])
...
def test_cleanup_rejects_positional_args(runner):
    ...
    result = runner.invoke(cli, ["crew", "cleanup", "peer-1000"])
...
def _prep_crew_with_peers(configure_hive_home, monkeypatch, peer_indices):
    """Set up a fake crew with main team `dev-0` + peers at the given indices.
    ...
    # Tag current pane as orch of main crew team (using a real crew instance
    # name from the canonical pool; legacy literal "crew" is rejected).
...
def test_cleanup_from_main_kills_all_peers(runner, configure_hive_home, monkeypatch):
    """Main crew pane + 2 peer teams → cleanup kills 2 windows, emits JSON,
    and leaves the main window's @hive-team intact.
    """
    expected = _prep_crew_with_peers(configure_hive_home, monkeypatch, [1000, 1001])
    killed = _stub_kill_window(monkeypatch)

    import hive.cli as cli_mod

    result = runner.invoke(cli, ["crew", "cleanup"])
    assert result.exit_code == 0, result.output

    payload = json.loads(result.output)
    assert sorted(payload["killedWindows"]) == sorted(expected)
    assert sorted(payload["killedTeams"]) == ["dev-0-cell-1000", "dev-0-cell-1001"]
...
def test_cleanup_with_no_peers_succeeds_with_empty_arrays(
    ...
    _prep_crew_with_peers(configure_hive_home, monkeypatch, [])
    killed = _stub_kill_window(monkeypatch)
...
def test_cleanup_rejected_from_peer_pane(runner, configure_hive_home, monkeypatch):
    """Running cleanup from a peer pane (team name ending in -cell-<N>) is
    ...
    # Simulate peer pane: group=<crew> but team name is <main>-cell-<N>.
    cli_mod.tmux.tag_pane("%512", "agent", "peaky.worker-1000", "dev-0-cell-1000", group="peaky")
    # Current pane's window is dev:0 per fixture; bind it to the peer team so
    # _resolve_scoped_team picks the peer name up.
    cli_mod.tmux.set_window_option("dev:0", "@hive-team", "dev-0-cell-1000")
    cli_mod.tmux.set_window_option("dev:0", "@hive-workspace", "/tmp/ws")
    ...
    result = runner.invoke(cli, ["crew", "cleanup"])
    assert result.exit_code != 0
    # Fail message calls out cell-team context.
    assert "cell" in result.output.lower()
    # Critically: no tmux window got killed on rejection.
    assert killed == []


def test_cleanup_rejected_from_non_crew_pane(runner, configure_hive_home, monkeypatch):
    """Pane without @hive-group=crew is rejected — e.g. a plain daily agent pane."""
    ...
    result = runner.invoke(cli, ["crew", "cleanup"])
    assert result.exit_code != 0
    assert "crew" in result.output.lower()
    assert killed == []
