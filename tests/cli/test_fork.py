...
def test_fork_uses_claude_profile_from_runtime_session(runner, configure_hive_home, monkeypatch):
    ...
    monkeypatch.setattr("hive.cli.detect_profile_for_pane", lambda _pane: type("P", (), {"name": "claude", "resume_cmd": "claude -r {session_id} --fork-session", "fork_needs_tui": False})())
    ...
    monkeypatch.setattr("hive.cli.tmux.kill_pane", lambda _pane: None)
    monkeypatch.setattr("hive.cli.tmux.wait_for_text", lambda *_args, **_kwargs: (_ for _ in ()).throw(AssertionError("claude should not wait for TUI fork text")))
...
def test_fork_falls_back_to_codex_resume_command(runner, configure_hive_home, monkeypatch):
    ...
    monkeypatch.setattr("hive.cli.detect_profile_for_pane", lambda _pane: type("P", (), {"name": "codex", "resume_cmd": "codex fork {session_id}", "fork_needs_tui": False})())
