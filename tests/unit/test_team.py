...
def test_adopting_a_lead_with_a_job_record_renames_the_job(configure_hive_home, monkeypatch):
    configure_hive_home(tmux_inside=True, current_pane="%7")
    renamed = []
    monkeypatch.setattr("hive.agent.detect_current_session_id", lambda _cwd, model="", pane_id="": "sess-123")
    monkeypatch.setattr("hive.team.tmux.get_current_session_name", lambda: "dev")
    monkeypatch.setattr("hive.team.tmux.get_current_window_target", lambda: "dev:0")
    monkeypatch.setattr("hive.team.tmux.tag_pane", lambda *args: None)
    monkeypatch.setattr("hive.team.tmux.enable_pane_border_status", lambda target: None)
    from hive.adapters import claude_bg

    monkeypatch.setattr(claude_bg, "job_id_for_pane", lambda pane: "abcd1234" if pane == "%7" else None)
    monkeypatch.setattr(claude_bg, "type_into_job", lambda job, text: renamed.append((job, text)))

    Team.create("honey", workspace="/tmp/ws")

    assert renamed == [("abcd1234", "/rename honey.orch")]


def test_adopting_a_lead_without_a_job_record_types_nothing(configure_hive_home, monkeypatch):
    configure_hive_home(tmux_inside=True, current_pane="%7")
    monkeypatch.setattr("hive.agent.detect_current_session_id", lambda _cwd, model="", pane_id="": "sess-123")
    monkeypatch.setattr("hive.team.tmux.get_current_session_name", lambda: "dev")
    monkeypatch.setattr("hive.team.tmux.get_current_window_target", lambda: "dev:0")
    ...
    monkeypatch.setattr(claude_bg, "job_id_for_pane", lambda pane: None)
    monkeypatch.setattr(
        claude_bg, "type_into_job",
        lambda job, text: (_ for _ in ()).throw(AssertionError("must not type")),
    )

    Team.create("honey", workspace="/tmp/ws")


def test_a_rename_failure_never_fails_team_init(configure_hive_home, monkeypatch):
    configure_hive_home(tmux_inside=True, current_pane="%7")
    monkeypatch.setattr("hive.agent.detect_current_session_id", lambda _cwd, model="", pane_id="": "sess-123")
    monkeypatch.setattr("hive.team.tmux.get_current_session_name", lambda: "dev")
    monkeypatch.setattr("hive.team.tmux.get_current_window_target", lambda: "dev:0")
    ...
    monkeypatch.setattr(claude_bg, "job_id_for_pane", lambda pane: "abcd1234")
    monkeypatch.setattr(
        claude_bg, "type_into_job",
        lambda job, text: (_ for _ in ()).throw(RuntimeError("attach wedged")),
    )

    team = Team.create("honey", workspace="/tmp/ws")
