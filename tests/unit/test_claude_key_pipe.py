...
def test_member_interrupt_without_a_job_record_is_refused(monkeypatch):
    """A lost job record leaves nothing addressable: Escape into the pane would
    land in whatever session its viewer is showing, so hive refuses instead."""
    _member_pane(monkeypatch, None)
    _forbid_tmux(monkeypatch)

    with pytest.raises(RuntimeError, match="no bg job record"):
        agent.Agent(name="red", team_name="probe", pane_id="%1", cli="claude").interrupt()
