...
13. 升级顺序：默认先在团队内讨论，不要先来问用户。需要评审、设计分歧、风险判断、卡住时，先用 `hive suggest` 找候选，再优先找 **非同源模型 / 非同 CLI** 的 agent 讨论，然后再 `hive send` 给对方。只有满足以下任一条件时才升级给用户：(a) 团队内没有合适 agent 可问；(b) 决策涉及不可逆外部副作用（`git push`、发 PR comment、删除数据、通知外部系统）；(c) 用户明确要求参与该类决策。
...
- Hive 不是严格可靠消息队列：没有幂等性或 backpressure；消息接收主通道是 pane 内联的 `<HIVE ...>` block，不要把 durable store 当收件箱。需要恢复上下文时，应依赖 hidden `hive delivery` / `hive thread` 和 workspace artifact
