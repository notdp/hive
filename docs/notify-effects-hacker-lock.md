...
Firing a notification and playing the animation are split by design. Fire time
sets only the durable signals and draws nothing; the animation runs after the
human selects the window, once per flash. The fire-time signals are the ones
that have to persist unattended.

The effect covers the whole pane, on a pane the human has just chosen to look
at. There is no theme switch and no opt-out. Disabling the `notify` plugin
silences the hived's idle watcher only; the manual `hive notify` is not
plugin-gated.
...
- **`python3` is an unshipped dependency.** The animation is the last thing on
  ...
  subcommands, this one was not. Nothing checks for an interpreter. A missing
  `python3` is a popup that opens and closes immediately (`-E`), and nothing
  records it: the attention script discards the popup's result and exits 0
  either way, so the `returncode` on the `attention.run` record reads the same
  for a failed play and a successful one.
- **The popup is client-local.** `display-popup` renders on one client. On a
  ...
  the animation; the others see the flash disappear.
- **Small panes overdraw.** The animation clamps its viewport to a minimum of
  50x16. In a pane narrower or shorter than that, the layout is computed for a
  viewport the pane does not have and the drawing runs past the pane edge.
