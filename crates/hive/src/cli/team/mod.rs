...
pub(crate) fn create(
        ...
        && crate::agent_cli::detect_profile_for_pane(&current_pane).is_some()
    {
        ...
        let result = ok_or_fail(create_orch_team(&current_pane, name));
...
/// Move the current orch pane into a team session without restarting its
/// viewer. The registry write commits the move; earlier errors swap back.
fn create_orch_team(current_pane: &str, name: &str) -> Result<Map<String, Value>> {
    gc_dead_teams();
    let existing = identity::discover_tmux_binding();
    if reuse_existing_binding(&existing, name).map_err(anyhow::Error::msg)? {
        return Ok(existing);
    }
    let source = crate::team_display::OrchSource::read(current_pane)?;
    let team_name = if name.is_empty() {
        crate::naming::pick_team_name(&source.session, &source.window, "0")
    } else {
        name.to_string()
    };
    let error = crate::team::validate_team_name(&team_name);
    if !error.is_empty() {
        bail!("{error}");
    }
    if crate::registry::load(&team_name).is_some() {
        bail!("team '{team_name}' already exists in the registry (hive delete {team_name} releases the name)");
    }
    crate::team_display::checked_team_session(&team_name)?;
    claim_team_name(&team_name, "", !name.is_empty())?;
    let ws_str = team_workspace(&team_name);
    crate::devlog::check_socket_path_len(Path::new(&ws_str)).map_err(anyhow::Error::msg)?;
    prepare_workspace(&team_name, &ws_str, false, &[])?;
    let description = format!("auto-init from tmux {} ({})", source.session, source.window);
    let cwd = source.cwd.clone();
    let orch_cli = crate::agent_cli::resolve_spawn_cli_name(None);
    let mut placement = source.place(&team_name)?;
    let committed = (|| -> Result<Team> {
        let t = Team::create_for_window(
            &team_name,
            &placement.window,
            current_pane,
        ...
        if t.tmux_window_id != placement.window_id
            || tmux::get_window_option(&placement.window_id, "hive-team").as_deref()
                != Some(&team_name)
        {
            bail!("team window disappeared while creating '{team_name}'");
        }
        placement.bind(&team_name, &orch_cli)?;
        let mut member = session_member_row(
            LEAD_AGENT_NAME,
            &orch_cli,
            t.lead_session_id.as_deref().unwrap_or_default(),
        );
        member.insert("cwd".into(), Value::String(cwd));
        let written = crate::registry::create_team(
            &t.name,
            &ws_str,
            &created_at_key(t.created_at),
            &[member],
        ...
        if written != "written" {
            bail!("team '{team_name}' was created concurrently; nothing was enrolled");
        }
        Ok(t)
    })();
    let mut t = match committed {
        Ok(t) => t,
        Err(error) => return Err(placement.rollback_error(error)),
    };
    // Saved context follows the commit; a failed create leaves the
    // caller's original context file intact.
    if let Err(error) =
        crate::context::save_context_for_pane(current_pane, &t.name, &ws_str, LEAD_AGENT_NAME)
    {
        eprintln!("hive: team registered; saving pane context failed: {error}");
    }
    remember_context(&t.name, &ws_str, LEAD_AGENT_NAME);
    start_team_hived_or_warn(&mut t, &ws_str);
    let window = placement.window.clone();
    let switched = placement.finish();
    ...
    orch.insert("pane".to_string(), Value::String(current_pane.to_string()));
    orch.insert(
        "name".to_string(),
    ...
    if !switched {
        result.insert(
            "nextStep".into(),
            Value::String(format!("hive attach {} opens the team window", t.name)),
        );
    }
    Ok(result)
}
...
fn claim_team_name(team_name: &str, this_window: &str, explicit: bool) -> Result<()> {
    ...
    if crate::team::window_has_live_team_members(&existing_wt, team_name) {
        ...
        bail!("team '{team_name}' already lives in tmux window '{existing_wt}' — {hint}.");
    }
