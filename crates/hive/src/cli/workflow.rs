//! `hive workflow run`: one workflow node, as the seam the `hive-node`
//! plugin agent runs in the background from a Claude Code Workflow script.
...
/// stderr only when the run never reached a dispatch (bad team, a claude
/// member, spawn or ready failure, the dispatch refused); every end past
/// that is a verdict in the JSON line and exit 0.
