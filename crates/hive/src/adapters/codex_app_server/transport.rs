...
fn urandom(n: usize) -> io::Result<Vec<u8>> {
...
pub(crate) fn ws_send_frame(stream: &UnixStream, opcode: u8, payload: &[u8]) -> io::Result<()> {
    let n = payload.len();
...
impl WsConn {
    ...
    fn handshake(&mut self) -> io::Result<()> {
        ...
        if find(&data[..head_end], b"101").is_none() {
            return Err(io::Error::new(
    ...
    fn recv_frame(&mut self) -> io::Result<(bool, u8, Vec<u8>)> {
        let header = self.recv_exact(2)?;
        ...
        if length == 126 {
            let bytes = self.recv_exact(2)?;
            length = u16::from_be_bytes([bytes[0], bytes[1]]) as u64;
        } else if length == 127 {
            let bytes = self.recv_exact(8)?;
            let mut raw = [0u8; 8];
        ...
        let mut payload = self.recv_exact(length as usize)?;
        if masked {
    ...
    pub fn recv_text(&mut self) -> io::Result<String> {
        ...
        loop {
            ...
            if opcode == 0x9 {
                ws_send_frame(&self.stream, 0xA, &payload)?;
                continue;
            }
            if opcode == 0xA {
                continue;
    ...
    pub fn send_text(&self, text: &str) -> io::Result<()> {
        ws_send_frame(&self.stream, 0x1, text.as_bytes())
    }

    pub fn close(&self) {
        let _ = ws_send_frame(&self.stream, 0x8, b"");
