...
/// One status tick over *panes*, the tick's `list-panes -a` snapshot. Only
/// `agent` panes get the two pane options — a mirror's `hive view` repaint
/// would read as output, and a mirror or terminal pane has no chip to mark
