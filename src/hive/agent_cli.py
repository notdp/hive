...
AGENT_CLI_CHOICES = ("claude", "codex", "grok")
...
# Anti-homogeneous peer CLI mapping. Different model families produce more
# diverse viewpoints than same-family pairs. claude↔codex stays the default
# pair so existing teams do not flip; grok pairs with claude.
_ANTI_PEER_CLI = {"claude": "codex", "codex": "claude", "grok": "claude"}


def anti_peer_cli(current_cli: str) -> str:
    """Return the anti-family peer CLI for *current_cli*."""
...
PROFILES: dict[str, CLIProfile] = {
    ...
    "grok": CLIProfile(
        name="grok",
        ready_text="Grok",
        fork_cmd="grok -r {session_id} --fork-session",
        skill_cmd="/{name}",
    ),
}
