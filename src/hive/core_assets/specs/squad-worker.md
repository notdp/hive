...
- **第一动作**:`hive worktree start <feature-id>` 并**进入 / 证明入场**:claude 用 `EnterWorktree path=<路径>`;codex / droid 后续每条 repo 命令都把 working directory 设为该 worktree,并先记录 entry proof(入场证明):`pwd`、`git rev-parse --show-toplevel`、`git status --short --branch`。base 自动解析到集成分支。之后的探索 / plan / 实现全在 worktree 里。
- **再把 plan 草案(带 worktree 路径;codex / droid 还要附 entry proof 输出)发 validator**(它对照 orch 的 VAL 挑;squad 里 VAL 已定稿,不重写),plan 过了再动手。plan 阶段**零上行** —— plan pass 不发 challenger / orch。
