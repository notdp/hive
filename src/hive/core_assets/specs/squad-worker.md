...
hive skills get duo    # worker 角色内核:plan 草案 → 实现 → handoff → 按 fail 迭代(含 handoff schema)
```
...
- owner = `<squad>.orch`;peer + 唯一下游 = `<squad>.validator-<N>`。
- 收到 orch 的 `<HIVE ... artifact=<path>>` → 直接 Read `artifact=` 全文,再读 `features.json` 对应条目 + `val-feature-<id>.md`(做什么、什么算做完)。
- **先把 plan 草案发 validator**(它对照 orch 的 VAL 挑;squad 里 VAL 已定稿,不重写),plan 过了再动手。
- handoff 写 `<workspace>/artifacts/handoffs/feature-<id>-handoff.md`(多次用 `feature-<id>-<ts>.md`),发 `hive send <squad>.validator-<N> "verify feature=<id>" --artifact <handoff>`。
