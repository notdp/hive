...
3. 然后只等消息:orch 的征询、或 worker 的终态交付。**出生 idle 纪律**(别 sleep / 翻库找活、读完就停、超 60s 才 ping 一次)统一见 `hive skills get core` 的「没活干时」;你的 idle ping 发 orch:`hive send <squad>.orch "<squad>.challenger idle, awaiting dispatch"`。
