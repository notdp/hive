...
2. 先跑幂等的 `hive init`，再用 `hive team` 确认 `team/runtimeWorkspace/self` 齐全；若 `hive team` 返回 `team: null`，回头补一次 `hive init`。
...
2. 再执行 `hive team` 验证 `team/runtimeWorkspace/self` 齐全(`self` 是你自己的 member name 字符串)
...
```json
{
  ...
  "hint": "No team bound. The duo-vs-squad choice is the user's — ask them with the blocking question tool, then init accordingly. Don't pick or init on your own.",
  "runtimeWorkspace": "/tmp/hive-<session>-<slug>",
  "cwd": "/absolute/path/to/repo"
}
```

说明第一步的 `hive init` 没落成,需要回头再跑一次 `hive init`,直到 `hive team` 返回 bound payload（含 `self` + `runtimeWorkspace` 指向团队 workspace）再继续。
...
| 命令 | 用途 |
| ---- | ---- |
| `hive team` | 查看团队成员 + 当前 pane 的 `self`(字符串 = 你自己的 member name;去 members 里按 name 查自己那行) |
| `hive init` | 初始化 team |
